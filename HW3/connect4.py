...
def minimax(player, board, depth_limit):
    ...
    def max_value(player, board, depth_limit):
        ...
        for col, child_board in get_child_boards(player, board):
            child_value = value(next_player, child_board, depth_limit-1)
            if child_value > v:
                v = child_value
                placement = col
    ...
    def min_value(player, board, depth_limit):
        ...
        for _, child_board in get_child_boards(player, board):
            child_value = value(next_player, child_board, depth_limit-1)
            v = min(v, child_value)
...
def alphabeta(player, board, depth_limit):
    ...
    def max_value(player, board, depth_limit, alpha, beta):
        ...
        for col, child_board in get_child_boards(player, board):
            child_value = value(next_player, child_board, depth_limit-1, alpha, beta)
            if child_value > v:
                v = child_value
                placement = col
            alpha = max(alpha, v)
...
def expectimax(player, board, depth_limit):
    ...
    def value(player, board, depth_limit):
        ...
        if player == max_player:
            ...
            return expect_value(player, board, depth_limit)
    ...
    def expect_value(player, board, depth_limit):
        
        nonlocal placement
        v = 0

        child_boards = get_child_boards(player, board)
        p = 1/len(child_boards)

        for _, child_board in child_boards:
            child_value = value(next_player, child_board, depth_limit-1)
            v += child_value * p
